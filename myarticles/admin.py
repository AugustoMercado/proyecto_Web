...
class ProductsAdmin(SortableAdminMixin, ImportExportModelAdmin):
    """
    Admin interface for managing products with sorting, import/export, and visual indicators.
    """
    ...
    def display_image(self, obj):
        ...
        if obj.image:
            return format_html(
                '<img src="{}" style="width: 50px; height: 50px; object-fit: contain; border-radius: 5px; border: 1px solid #ddd;" />',
                obj.image.url
            )
        return "No Image"
    ...
    def stock_status(self, obj):
        """
        Displays a colored label indicating current stock levels.
        """
        if obj.stock == 0:
...
class CategoryAdmin(SortableAdminMixin, admin.ModelAdmin):
    """
    Admin interface for managing product categories with sorting capability.
    """
    
    list_display = ('name', 'is_active', 'my_order')
